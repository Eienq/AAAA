...
let top = message.guild.members.cache.filter(uye => db.get(`toplamistatistik${uye.id}`)).array().sort((uye1, uye2) => Number(db.get(`toplamistatistik${uye2.id}`))-Number(db.get(`toplamistatistik${uye1.id}`))).slice(0, 15).map((uye, index) => (index+1)+" • <@"+ uye +"> | \`" + db.get(`yetkili.${uye.id}.toplam`) +"\` Kayıta Sahip.").join('\n');
